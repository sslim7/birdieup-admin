...
- **지표 대시보드** (`/`, 권한 키 `metrics`): 홈이 곧 대시보드다. KPI 카드·일별 추이·구성 차트·
  일자별 표(CSV 내보내기). 예전의 메뉴 바로가기 격자는 **사이드바와 중복이라 걷어냈다**
...
지표 화면은 **누적값(실시간 집계)과 추이(매일 04:00 KST 배치가 접어 둔 전날까지의 값)를 구분해서**
보여준다. 아직 집계되지 않은 날(대개 오늘)과 데이터가 없는 날은 0 이 아니라 `—` 로 그린다 —
0 으로 그리면 "오늘 서비스가 죽었다"로 읽히기 때문이다. 계약은 `docs/admin-api.md` §8.
...
- 예외가 하나 있다: `metrics`. **홈(대시보드)에는 권한을 달지 않는다** — 라우트 가드가 권한 없는
  경로를 홈으로 되돌리는데 그 홈까지 막히면 무한 리다이렉트가 된다. 화면은 누구나 열 수 있고
  막히는 것은 지표 API 뿐이며, 403 이면 안내와 접근 가능한 메뉴 목록으로 대체된다
...
│   └── Dashboard.jsx    # 홈 = 지표 대시보드 (메뉴 바로가기가 아니다)
├── components/
...
├── lib/metrics.js       # 지표의 "0 과 없음" 판정·KST 날짜 계산·CSV (표·차트·CSV 가 공유)
...
차트는 `recharts` 2.x 로 그린다. 3.x 는 React 19 대상이라 이 저장소(React 18.3)에서 쓰지 않는다.
